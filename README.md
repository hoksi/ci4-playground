...
- **49개 예제** — 기초부터 실무 패턴까지 전 범위 커버
- **상단 드롭다운 네비게이션** — 6개 카테고리 드롭다운 + 예제 페이지 하단 이전/다음 버튼
...
| 47 | **파일 업로드 심화** | Drag & Drop, FileReader 이미지 미리보기, XHR `upload.onprogress` 진행률, AJAX 업로드·삭제 |
...
| 42 | **CI4 공식 Queue** | `codeigniter4/queue` 패키지 — Database 핸들러, 우선순위, 재시도 |
...
| 45 | **Server-Sent Events** | SSE 실시간 스트림 — EventSource, 커스텀 이벤트, 자동 재연결, 실시간 큐 현황 모니터링 |
| 46 | **알림 시스템** | DB 기반 알림 생성·읽음 처리·전체 읽음 + SSE 실시간 미읽음 배지 업데이트 |
| 49 | **AG Grid** | AG Grid Community + CI4 JSON API — 클라이언트 사이드 정렬·필터·페이지네이션·CSV, 서버 사이드 Infinite Row Model |
...
└── Views/
    ├── layouts/main.php              ← 공통 레이아웃 — 상단 드롭다운 네비게이션, 이전/다음 버튼
    ├── home/index.php                ← 목차 페이지
    └── examples/                     ← 49개 예제 뷰
resources/
└── fonts/
    └── NotoSansKR-Regular.ttf       ← 한글 PDF 폰트 (NotoSansCJK KR, git 추적)
writable/
├── fonts/                            ← DOMPDF UFM 메트릭 캐시 (런타임 생성, .gitignore)
├── uploads/                          ← 업로드 파일 저장소
│   ├── advanced/                     ← 파일 업로드 심화 예제 업로드 파일
│   └── tinymce/                      ← TinyMCE 에디터 이미지 업로드
